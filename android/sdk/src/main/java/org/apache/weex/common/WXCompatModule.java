...
public abstract class WXCompatModule extends WXModule implements Destroyable {
  ...
  public WXCompatModule() {
    ...
    LocalBroadcastManager.getInstance(WXEnvironment.getApplication())
        .registerReceiver(mModuleReceive, new IntentFilter(ACTION_ACTIVITY_RESULT));
    LocalBroadcastManager.getInstance(WXEnvironment.getApplication())
        .registerReceiver(mModuleReceive, new IntentFilter(ACTION_REQUEST_PERMISSIONS_RESULT));
  }
    ...
    public void onReceive(Context context, Intent intent) {
      ...
      switch (action) {
        case ACTION_ACTIVITY_RESULT:
          ...
          break;
        case ACTION_REQUEST_PERMISSIONS_RESULT:
