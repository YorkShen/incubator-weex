...
//      }
    ...
    post(new Runnable() {
      ...
      public void run() {
        ...
        if (null != instance && instance.isPreInitMode()){
          ...
          instance.getApmForInstance().onStageWithTime(WXInstanceApm.KEY_PAGE_STAGES_END_EXCUTE_BUNDLE,
              WXUtils.getFixUnixTime()+600);
        }
      }
    });
  }
  ...
  private WXParams assembleDefaultOptions() {
    ...
    wxParams.setDeviceWidth(TextUtils.isEmpty(config.get("deviceWidth")) ? String.valueOf(
        WXViewUtils.getScreenWidth(WXEnvironment.sApplication)) : config.get("deviceWidth"));
  ...
  public void setMargin(String instanceId, String ref, EDGE edge, float value) {
    if (isSkipFrameworkInit(instanceId) || isJSFrameworkInit()) {
      mWXBridge.setMargin(instanceId, ref, edge, value);
    }
  }

  public void setPadding(String instanceId, String ref, EDGE edge, float value) {
    if (isSkipFrameworkInit(instanceId) || isJSFrameworkInit()) {
      mWXBridge.setPadding(instanceId, ref, edge, value);
    }
  }

  public void setPosition(String instanceId, String ref, EDGE edge, float value) {
    if (isSkipFrameworkInit(instanceId) || isJSFrameworkInit()) {
